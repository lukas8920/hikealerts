...
public class EventController {
    ...
    @CrossOrigin
    @GetMapping("/query")
    public ResponseEntity<List<EventResponse>> queryEvents(@Parameter(description = "Bounding box to query events where the mid point of the trail lies within the box. The boundary box is defined by the minimum longitude, the minimum latitude, the maximum longitude and the maximum latitude.", example = "boundary=1.14,1.14,4.8,4.8", required = false) @RequestParam(name = "boundary", required = false) String boundary,
                                                           @Parameter(description = "Country in ISO 3166-1 alpha-2 format. Valid countries are NZ or US. Either a boundary box or a country is required for the query.", example = "country=NZ", required = false) @RequestParam(name = "country", required = false) String country,
                                                           @Parameter(description = "From date which specifies when the event becomes valid (might be not given by the event provider). The date should be in format yyyy-MM-dd.", example = "fromDate=2024-11-05", required = false) @RequestParam(name = "fromDate", required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fromDate,
                                                           @Parameter(description = "To date which specifies when the event becomes invalid (might be not given by the event provider). The date should be in format yyyy-MM-dd", example = "toDate=2024-12-05", required = false) @RequestParam(name = "toDate", required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate toDate,
                                                           @Parameter(description = "Create date which specifies when the event was provided by data provider. Returns all events greater or equal to the create date. The date should be in format yyyy-MM-dd", example = "createDate=2024-11-09", required = false) @RequestParam(name = "createDate", required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate createDate,
                                                           ...
                                                           @Parameter(description = "Filters on who created the events. Valid values are: 'All', 'Community' or 'Official'. Default value is 'All'.", example = "createdBy=Official", required = false) @RequestParam(name = "createdBy", required = false, defaultValue = "All") String createdBy,
                                                           @Parameter(description = "The offset for the results in the event datasource. Enables paging through the events. Default value is 0", example = "offset=50", required = false) @RequestParam(name = "offset", required = false, defaultValue = "0") int offset,
                                                           @Parameter(description = "The limit defining the maximum number of results to return. Default value is 50. Maximum value is 100.", example = "limit=40", required = false) @RequestParam(name = "limit", required = false, defaultValue = "50") int limit) throws BadRequestException {
    ...
    public ResponseEntity<MessageResponse> publishEvent(@Parameter(description = "A brief title for the event which may not exceed 200 words. In order to allow geospatial mapping either the description or the title should mention the trail name.", example = "Track closure", required = true) @RequestParam(name = "title", required = true) String title,
                                                        @Parameter(description = "A description of the event which may not exceed 2000 words. In order to allow geospatial mapping either the description or the title should mention the trail name.", example = "Mount Elvis track closed due to lumbering operations.", required = true) @RequestParam(name = "description", required = true) String description,
                                                        @Parameter(description = "Country in ISO 3166-1 alpha-2 format. Valid countries are NZ or US.", example = "country=NZ", required = true) @RequestParam(name = "country", required = true) String country,
                                                        @Parameter(description = "From date which specifies when the event becomes valid (might be not given by the event provider). The date should be in format yyyy-MM-dd.", example = "fromDate=2024-11-05", required = false) @RequestParam(name = "fromDate", required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fromDate,
                                                        @Parameter(description = "To date which specifies when the event becomes invalid (might be not given by the event provider). The date should be in format yyyy-MM-dd", example = "toDate=2024-12-05", required = false) @RequestParam(name = "toDate", required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate toDate) throws BadRequestException, AiException {
    ...
    @CrossOrigin
    @PostMapping("/delete")
    public ResponseEntity<MessageResponse> deleteEvent(@Parameter(description = "The event of the id. Id's can be retrieved via the query endpoint.", example = "id=1", required = true) @RequestParam(name = "id", required = true) String id) throws BadRequestException {
